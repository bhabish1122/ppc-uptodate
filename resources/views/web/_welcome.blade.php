...
@section('content')
    ...
    <div class="container">
      <div class="row border-responsive">
        ...
        <div class="col-md col-lg mb-4 mb-lg-0 border-right" data-aos="fade-up" data-aos-delay="400">
          <a href="{{ route('web.notice.index', 'covid-notice-board') }}" class="text-center">
            <span class="fas fa-align-left display-4 d-block mb-3 text-danger"></span>
            <h3 class="text-uppercase h4 mb-3">{{ __('lang.covid_notice_board')}}</h3>
          </a>
        </div>
  ...
  <!-- about -->
  @if(count($abouts))
  <div class="site-section {{ $cores_subs->count() ? 'p-0': 'pt-0' }}">
    <div class="container">
      <div class="row">
        <div class="col-lg-{{$cores ? '7' : '12'}}">
          @foreach($abouts as $about)
          <div class="no-gutters align-items-stretch">
            <div data-aos="fade-up" data-aos-delay="100">
              <span class="caption d-block mb-2 font-secondary font-weight-bold">Welcome To</span>
        ...
        @if ($cores)
        <div class="col-lg-5 main-mt-sm-4 mt-xs-4">
          <div class="mb-5" data-aos="fade-up" data-aos-delay="300">
            ...
            <div class="user-profile-container">
              <div class="user-profile text-light" style="background-image: url('{{URL::to('/')}}/image/core_person/{{$cores->image_enc}}');">
                <b class="bg-dark px-3 rounded-top">{{$cores->name}}</b>
              </div>
              <ul class="user-info px-4 text-center">
                <li>
                  <h4>{{$cores->designation}}</h4>
                </li>
              </ul>
            </div>
              </a>
          ...
          <div class="tile" id="tile-1" data-aos="fade-up" data-aos-delay="100">
            ...
            <div class="tab-content">
                    ...
                    <div class="coupon bg-light rounded px-3 mb-3 d-flex justify-content-between border-dark coupon-shadow" data-toggle="tooltip" data-placement="right" data-html="true" title="{!! $general_list->image_enc ? '<small>'.__('lang.download').'</small>' : '' !!}">
                      <div class="py-3 w-100 justify-content-start">
                        <div class="d-block mr-2">
                          <h3 class="lead" data-toggle="tooltip" data-placement="top" data-html="true" title="<small>{{$general_list->title}}</small>">{{ substr($general_list->title,0, 75) }}</h3>
                  ...
                  <table class="table table-hover table-striped table-bordered">
                    ...
                    <tbody class="text-center">
                      ...
                      <tr data-toggle="tooltip" data-placement="right" data-html="true" title="<small>{{$posting_list->title}}</small>">
                        ...
                        <td class="text-left">{{ substr($posting_list->title, 0,35) }}</td>
                        <td class="text-danger">
              ...
              <div class="tab-pane fade" id="notice-board" role="tabpanel" aria-labelledby="notice-board-tab">
                <div class="no-gutters align-items-stretch mt-4">
                  ...
                  <div data-aos="fade-up" data-aos-delay="{{$key+1}}00">
                    <div class="coupon bg-light rounded px-3 mb-3 d-flex justify-content-between border-dark coupon-shadow" data-toggle="tooltip" data-placement="right" data-html="true" title="<small>{{ __('lang.download')}}</small>">
                      <div class="py-3 w-100 justify-content-start">
                        <div class="d-block mr-2">
                          <h3 class="lead" data-toggle="tooltip" data-placement="top" data-html="true" title="<small>{{$bulletine_list->title}}</small>">{{ substr($bulletine_list->title,0,35) }}</h3>
                          <div class="text-muted mb-0">
                ...
                <div class="float-right">
                  <a class="effect1" href="{{ route('web.notice.index', 'covid-notice-board') }}">
                    {{ __('lang.explose_more')}}
                    <span class="bg"></span>
                  </a>
                </div>
                ...
                @forelse ($document_actregulation as $key=>$document_regulation)
                  <div data-aos="fade-up" data-aos-delay="{{$key+1}}00">
                    <div class="coupon bg-light rounded px-3 mb-3 d-flex justify-content-between border-dark coupon-shadow" data-toggle="tooltip" data-placement="right" data-html="true" title="<small>{{ __('lang.download')}}</small>">
                      <div class="py-3 w-100 justify-content-start">
                        <div class="d-block mr-2">
                          <h3 class="lead" data-toggle="tooltip" data-placement="top" data-html="true" title="<small>{{$document_regulation->title}}</small>">{{ substr($document_regulation->title, 0,75) }}</h3>
                          <div class="text-muted mb-0">
                ...
                @forelse ($citizen_charters as $key=>$citizen_charter)
                  <div data-aos="fade-up" data-aos-delay="{{$key+1}}00">
                    <div class="coupon bg-light rounded px-3 mb-3 d-flex justify-content-between border-dark coupon-shadow" data-toggle="tooltip" data-placement="right" data-html="true" title="<small>{{ __('lang.download')}}</small>">
                      <div class="py-3 w-100 justify-content-start">
                        <div class="d-block mr-2">
                          <h3 class="lead" data-toggle="tooltip" data-placement="top" data-html="true" title="<small>{{$citizen_charter->title}}</small>">{{ substr($citizen_charter->title,0, 75) }}</h3>
                          <div class="text-muted mb-0">
  ...
  <div class="site-section {{$galleryhasimages->count() ? 'bg-primary py-4' : 'pb-5 pt-0'}}">
    <div class="container">
      <div class="row">
        @foreach($corepersons as $key=>$coreperson)
        <div class="col-md-4">
          <div class="card text-center" data-aos="fade-{{ $key == 0 ? 'right' : 'left'}}" data-aos-delay="{{ $key+1 }}00" >
            ...
            <a href="{{ route('web.about.list-of-director-generals.show',$coreperson->id) }}" class="card-body" data-toggle="tooltip" data-placement="top" data-html="true" title="<small>
              ...
              <p class="card-text">{{ substr($coreperson->designation, 0,30) }}</p>
              <h5 class="card-title">{{$coreperson->phone}}</h5>
              <h5 class="card-title">{{$coreperson->email}}</h5>
            </a>
          </div>
        </div>
        {{-- <div class="col-md-4">
          <div class="blog-card text-dark" data-aos="fade-{{ $key == 0 ? 'right' : 'left'}}" data-aos-delay="{{ $key+1 }}00" data-toggle="tooltip" data-placement="top" data-html="true" title="<small>
            ...
            <div class="description text-sm-center-main">
              ...
              <p>{{ substr($coreperson->designation,0, 30) }}</p>
              <ul class="list-unstyled list">
                <li class="mb-2 d-flex align-items-center"><i class="fa fa-phone mr-3 text-danger"></i> {{ substr($coreperson->phone,0, 26) }}</li>
                <li class="mb-2 d-flex align-items-center" data-toggle="tooltip" data-placement="left" data-html="true" title="Fax"><i class="fa fa-fax mr-3 text-danger"></i> {{ substr($coreperson->fax, 26) }}</li>
                <li class="mb-2 d-flex align-items-center"><i class="far fa-paper-plane mr-3 text-danger"></i>{{ substr($coreperson->email,0, 26) }}</li>
              </ul>
            </div>
          </div>
        </div> --}}
