...
class BillSarwajanikaranController extends Controller
{
    public function index(Request $request){
      return view('admin.bill_sarwajanikaran.index');
    }

    public function create()
    {
        return view('admin.bill_sarwajanikaran.create');
    }
    ...
    {  
      ...
      $pass = array(
          'message' => ' BillSarwajanikaran Created',
          'alert-type' => 'success'
      );
      return redirect()->route('admin.bill_sarwajanikaran.index')->with($pass);
    }

    public function edit($id){
      $bill_sarwajanikarans = OrganizationalStructure::findOrFail($id);
      return view('admin.bill_sarwajanikaran.edit', compact('bill_sarwajanikarans'));
    }
    ...
    {   
      ...
      $pass = array(
          'message' => 'BillSarwajanikaran Updated!',
          'alert-type' => 'success'
      );
      return redirect()->route('admin.bill_sarwajanikaran.index')->with($pass);
    }
    ...
    public function destroy($id){
      ...
      if($usertype->delete()){
        $pass = array(
            'message' => 'BillSarwajanikaran Deleted!',
            'alert-type' => 'success'
        );
      }else{
        $pass = array(
            'message' => 'BillSarwajanikaran could not be Deleted!',
            'alert-type' => 'error'
        );
    ...
    public function getBillSarwajanikaranList(Request $request)
    {

      $data = OrganizationalStructure::where('page',2)->orderBy('id','DESC');
      $data = $data->get();            
      ...
      ->addColumn('is_active', function($row){
          ...
          $action = '<a href="'.route('admin.bill_sarwajanikaran.status',[$row->id,$row->is_active]).'" class="btn btn-sm btn-outline-danger" data-toggle="tooltip" data-placement="top" >';
      ...
      ->addColumn('action', function($row){
          // dd($row->id);
        $action = '<a href="'.route('admin.bill_sarwajanikaran.edit',$row->id).'" class="btn btn-sm btn-flat btn-outline-primary"><i class="fas fa-pencil-alt" title="Click to edit"></i></a> 
        <a href="#" data-url="'.route('admin.bill_sarwajanikaran.destroy', $row->id).'" class="btn btn-sm btn-outline-danger " onclick="return confirmDelete(event);"><i class="fa fa-trash" data-url="'.route('admin.bill_sarwajanikaran.destroy', $row->id).'"></i></a>
        ';
        return $action;
